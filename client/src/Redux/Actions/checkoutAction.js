...
import * as actionType from "../Constants/checkoutConstant";

export const addToCheckout = (id, quantity) => async (dispatch, getState) => {
  try {
    const config = {
      headers: {
    ...
    if (existingItem) {
      ...
      dispatch({ type: actionType.ADD_TO_CHECKOUT_SUCCESS, payload: { ...productDetails, quantity } });
    }
  } catch (error) {
    console.log(error);
    dispatch({ type: actionType.ADD_TO_CHECKOUT_ERROR, payload: error.message });
  }
};

export const deleteFromCart = (id) => async (dispatch) => {
  dispatch({ type: actionType.REMOVE_FROM_CHECKOUT, payload: id });
};
